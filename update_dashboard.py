...
def get_market_data():
    ...
    for i, indicator in enumerate(indicators, 1):
        print(f"\n[{i}/{len(indicators)}] {indicator['name']} ({indicator['symbol']}) 수집 중...")
        ...
        try:
            ...
            success_count += 1
            print(f"  ✓ 성공: {current_price:,.2f} ({change_percent:+.2f}%)")
    ...
    print(f"\n{'='*60}")
    print(f"수집 완료: {success_count}/{len(indicators)}개 성공")
    print(f"{'='*60}\n")
